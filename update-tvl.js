...
const fetchVaultsTvl = async (prices) => {
  let url = `https://stacks-node-api.mainnet.stacks.co/extended/v1/address/SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-stacker-v1-1/balances`;
  let response = await fetch(url, { credentials: 'omit' });
  let data = await response.json();
  let sum = (data['stx']['balance'] / 1000000) * (prices['stx'] / 1000000);
  
  url = `https://stacks-node-api.mainnet.stacks.co/extended/v1/address/SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-stacker-2-v1-1/balances`;
  response = await fetch(url, { credentials: 'omit' });
  data = await response.json();
  sum += (data['stx']['balance'] / 1000000) * (prices['stx'] / 1000000);

  url = `https://stacks-node-api.mainnet.stacks.co/extended/v1/address/SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-stacker-3-v1-1/balances`;
  response = await fetch(url, { credentials: 'omit' });
  data = await response.json();
  sum += (data['stx']['balance'] / 1000000) * (prices['stx'] / 1000000);

  url = `https://stacks-node-api.mainnet.stacks.co/extended/v1/address/SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-stacker-4-v1-1/balances`;
  response = await fetch(url, { credentials: 'omit' });
  data = await response.json();
  sum += (data['stx']['balance'] / 1000000) * (prices['stx'] / 1000000);
